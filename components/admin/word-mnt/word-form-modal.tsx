...
export function WordFormModal({ open, word, onClose, onSave }: Props) {
	const {
		...
		reset,
		formState: { errors },
	} = useForm<Word>({ defaultValues: emptyWord() });
	...
	const submit = (draft: Word) => {
		...
		if (cleaned.meanings.length) onSave(cleaned);
		else return;
	};
	return (
		<Modal
			...
			<form
				...
				<div className='flex-1 space-y-6 overflow-y-auto px-6 py-5'>
					...
					<section className='flex flex-col gap-3'>
						<div className='flex items-center justify-between'>
							...
							<button
								type='button'
								onClick={() =>
									meanings.append({ pos: 'noun', meaning: '', example: '' })
								}
							...
							<div
								...
								<div className='grid grid-cols-1 gap-3 sm:grid-cols-[8rem_1fr_auto]'>
									<Field
										...
										{...register(`meanings.${index}.pos` as const)}>
										<select>
									...
									<button
										...
										className='mt-6 flex size-11 items-center justify-center rounded-xl hover:bg-destructive/10'>
					...
					<ListEditor
						title='Phrases'
						fields={phrases.fields}
						append={() => (phrases.append as (value: string) => void)('')}
						remove={phrases.remove}
						register={register}
						name='phrases'
					...
					<ListEditor
						...
						append={() => (synonyms.append as (value: string) => void)('')}
						remove={synonyms.remove}
						register={register}
						name='synonyms'
...
function ListEditor({
	...
	register: ReturnType<typeof useForm<Word>>['register'];
	name: 'phrases' | 'synonyms';
	placeholder: string;
}) {
	return (
		<section className='flex flex-col gap-3'>
			...
			{fields.map((item, index) => (
				<div
					...
					<button
						...
						className='mt-6 flex size-11 items-center justify-center rounded-xl'>
						<Trash2 className='size-4' />
					</button>
				</div>
			))}
