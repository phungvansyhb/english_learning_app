...
export default function SpeakingLibrary() {
	const [type, setType] = useState<SpeakingType | 'all'>('all');
	const [difficulty, setDifficulty] = useState<SpeakingDifficulty | 'all'>('all');
	const [page, setPage] = useState(1);
	const filtered = useMemo(
		() =>
			speakingLessons.filter(
				(x) =>
	...
	const choose = (next: SpeakingType | 'all') => {
		setType(next);
		setPage(1);
	};
	return (
			...
			<div className='mx-auto flex max-w-6xl flex-col gap-7'>
					...
					<div className='flex flex-col gap-6 md:flex-row md:items-end md:justify-between'>
						<div className='max-w-2xl'>
							...
							<p className='mt-3 text-pretty leading-6 text-muted-foreground'>
								Chuẩn bị ý, ghi âm câu trả lời và tự nghe lại để cải thiện độ trôi
								chảy mỗi ngày.
							</p>
						</div>
				...
				<section className='flex flex-col gap-4 rounded-xl border bg-card p-4 shadow-sm'>
					<div className='flex items-center gap-2 text-sm font-semibold'>
						<SlidersHorizontal className='size-4 text-primary' /> Chọn cách luyện tập
					</div>
					<div className='flex flex-col gap-3 md:flex-row md:items-center md:justify-between'>
						<div className='flex flex-wrap gap-2'>
							{speakingTypes.map((item) => (
								<button
									key={item}
									onClick={() => choose(item)}
									className={cn(
				...
				<div>
					<h2 className='text-xl font-bold'>Bài luyện đề xuất</h2>
						...
						<Link
							...
							<div className='flex items-start justify-between gap-3'>
								<span className='rounded-md bg-primary/10 px-2.5 py-1 text-xs font-bold text-primary'>
									{speakingTypeLabels[lesson.type]}
								</span>
