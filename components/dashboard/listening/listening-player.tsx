	...
	useEffect(() => {
		const onKeyDown = (event: KeyboardEvent) => {
			...
			if (event.key === 'ArrowRight')
				audio.currentTime = Math.min(audio.duration || duration, audio.currentTime + 5);
			if (event.key === 'ArrowUp') {
				event.preventDefault();
				const next = Math.min(1, audio.volume + 0.1);
				audio.volume = next;
				setVolume(next);
			}
	...
	const seek = (offset: number) => {
		if (audioRef.current)
			audioRef.current.currentTime = Math.max(
				0,
				Math.min(duration, audioRef.current.currentTime + offset),
			);
	};
			...
			<div className='mt-5 flex flex-wrap items-center justify-between gap-3'>
				<div className='flex items-center gap-2'>
					...
					<button
						...
						className='flex size-11 items-center justify-center rounded-full bg-primary text-primary-foreground shadow-sm hover:opacity-90'>
						{playing ? (
					...
					<button
						...
						className='rounded-lg border px-3 py-2.5 text-sm font-semibold hover:bg-secondary'>
						+5s
					</button>
				</div>
