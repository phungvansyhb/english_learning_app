...
type PopupState = { position: Position; entry: InsertVocabDataPayload } | null;
	...
	const openPopup = async () => {
		startTransition(async () => {
			...
			const response = await fetch('/api/dictionary/translate', {
				...
				body: JSON.stringify({ word: trigger.text }),
			});
			if (!response.ok) return;
			const { data } = (await response.json()) as {
				data: InsertVocabDataPayload;
			};
			...
			setPopup({ position: { top, left }, entry: data });
			setTrigger(null);
		});
	};
