...
import { getPartDescription, partLabels, ReadingLesson } from '@/lib/reading-data';

export default function ReadingExercise({ lesson }: { lesson: ReadingLesson }) {
	const [current, setCurrent] = useState(0);
	const [answers, setAnswers] = useState<Record<number, string>>({});
	const [checked, setChecked] = useState(false);
	const question = lesson.questions[current];
	const answered = Object.keys(answers).length;
	const progress = Math.round((answered / lesson.questions.length) * 100);
	...
	const go = (direction: number) => {
		setCurrent((value) =>
			Math.max(0, Math.min(lesson.questions.length - 1, value + direction)),
		);
		setChecked(false);
	};
			...
			<div className='mx-auto flex max-w-6xl flex-col gap-6'>
				<header className='flex items-center justify-between gap-4'>
					<Link
						...
						<ArrowLeft className='size-4' /> Thư viện Reading
					</Link>
					<div className='flex items-center gap-2 rounded-full bg-card px-3 py-2 text-xs font-medium text-muted-foreground'>
					...
					<div className='flex flex-col gap-4 md:flex-row md:items-start md:justify-between'>
						<div>
							...
							<p className='mt-2 text-sm text-muted-foreground'>
								{getPartDescription(lesson.part)} · {lesson.questions.length} câu
								hỏi
							</p>
						</div>
				...
				<div className='grid gap-6 lg:grid-cols-[minmax(0,1fr)_280px]'>
					<section className='flex flex-col gap-5'>
						{lesson.passage && (
							<article className='whitespace-pre-line rounded-xl border bg-card p-5 text-sm leading-7 shadow-sm md:p-7'>
						...
						<article className='rounded-xl border bg-card p-5 shadow-sm md:p-7'>
							...
							{lesson.part === 'part-6' && (
								<div className='mt-6 rounded-lg border border-dashed bg-secondary/50 p-4 text-sm leading-7 text-muted-foreground'>
									Chọn từ phù hợp để hoàn chỉnh nội dung email ở trên.
								</div>
							)}
							{question.options ? (
								...
								<textarea
									...
									className='mt-7 min-h-28 w-full resize-y rounded-xl border bg-background p-4 text-sm outline-none focus:ring-2 focus:ring-primary/30'
								/>
							)}
							{checked && (
								<div
							...
							<div className='mt-7 flex flex-wrap items-center justify-between gap-3 border-t pt-5'>
								...
								<button
									...
									<Send className='size-4' /> Kiểm tra
								</button>
								<button
									onClick={() => go(1)}
					...
					<aside className='flex flex-col gap-4'>
						<div className='rounded-xl border bg-card p-5 shadow-sm'>
							<div className='flex items-center gap-2 font-semibold'>
								<Lightbulb className='size-4 text-primary' /> Mẹo làm bài
							</div>
							<p className='mt-3 text-sm leading-6 text-muted-foreground'>
								{lesson.part === 'part-5'
									? 'Đọc toàn bộ câu trước khi nhìn đáp án. Xác định từ loại cần điền để loại nhanh các lựa chọn.'
									: lesson.part === 'part-6'
										? 'Đọc cả đoạn để nắm mạch ý. Đừng chỉ dựa vào một câu ngay trước chỗ trống.'
										: 'Đọc câu hỏi trước, sau đó quay lại passage để tìm từ khóa và thông tin liên quan.'}
							</p>
						</div>
						<div className='rounded-xl border bg-card p-5 shadow-sm'>
