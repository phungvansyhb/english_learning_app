...
export default function ReadingLibrary() {
	...
	const filtered = useMemo(
		() =>
			readingLessons.filter(
				(lesson) =>
					(part === 'all' || lesson.part === part) &&
	...
	const selectPart = (value: ReadingPart | 'all') => {
		setPart(value);
		setPage(1);
	};
	const selectDifficulty = (value: Difficulty | 'all') => {
		setDifficulty(value);
		setPage(1);
	};
			...
			<div className='mx-auto flex max-w-6xl flex-col gap-7'>
					...
					<div className='flex flex-col gap-5 md:flex-row md:items-end md:justify-between'>
						<div className='flex max-w-2xl flex-col gap-3'>
							...
							<p className='text-pretty leading-6 text-muted-foreground'>
								Chọn một bài đọc vừa sức, luyện theo từng Part và theo dõi tiến bộ
								của bạn qua mỗi phiên học.
							</p>
						</div>
				...
				<section className='flex flex-col gap-4 rounded-xl border bg-card p-4 shadow-sm md:flex-row md:items-center md:justify-between'>
					<div className='flex items-center gap-2 text-sm font-semibold'>
						<SlidersHorizontal className='size-4 text-primary' /> Bộ lọc bài đọc
					</div>
						...
						<div className='flex flex-wrap gap-2'>
							{parts.map((value) => (
								<button
									...
									{value === 'all' ? 'Tất cả Part' : partLabels[value]}
								</button>
							))}
						</div>
				...
				<div className='flex items-center justify-between'>
					<div>
						<h2 className='text-xl font-bold'>Bài đọc đề xuất</h2>
